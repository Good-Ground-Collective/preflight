...
import { rule } from '../../src/rules/no-unsafe-dictionary-type.js';
...
ruleTester.run('no-unsafe-dictionary-type', rule, {
  valid: [
    // A concrete value type is the whole point.
    ...
    // `unknown` nested inside the value type is not the value type itself.
    ...
    // A shadowed built-in is not the built-in.
    ...
    // Substitution resolves the default through another alias.
    'type Value<T> = T; type Index<T = Command, U = Value<T>> = Record<string, U>; type A = Index;',
    // An intersection with a real member is not empty.
    ...
    // Merged interface declarations can contribute members.
  ...
  invalid: [
    ...
    // Transparent wrappers are read through.
    {
    ...
    // `Pick`/`Omit` preserve the source's value type.
    {
    ...
    // Interface index signatures are covered.
    ...
    // A plain alias use reports at the alias, not at each use site.
    {
    ...
    // Type-parameter defaults are substituted before classification.
    {
